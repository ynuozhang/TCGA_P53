...
load("D:/TCGA_P53/data/Step1_selectedgene_HiSeqcounts.Rdata")

# FPKM trace back
cancer_type = 'BRCA'
#Group set 1 and 2

Group1and2 <- transform(merge(HiSeq_selected_patients_woTP53mut, HiSeq_selected_patients_withTP53mut, by=0, all=T),
                        row.names = Row.names, Row.names = NULL)
Group1and2.label <- c(rep("TP53_WT",times=dim(HiSeq_selected_patients_woTP53mut)[2]),
                      rep("TP53_mutant",times=dim(HiSeq_selected_patients_withTP53mut)[2]))
...
#Group1and3=======================================================================
Group1and3 <- transform(merge(HiSeq_selected_patients_woTP53mut, HiSeq_selected_patients_withMultipleTP53mut, by=0, all=T),
                        row.names = Row.names, Row.names = NULL)
Group1and3.label <- c(rep("TP53_WT",times=dim(HiSeq_selected_patients_woTP53mut)[2]),
                      rep("TP53_multiple_mutant",times=dim(HiSeq_selected_patients_withMultipleTP53mut)[2]))
...
#Group1and4============================================================
Group1and4 <- transform(merge(HiSeq_selected_patients_woTP53mut, HiSeq_selected_patients_R175, by=0, all=T),
                        row.names = Row.names, Row.names = NULL)
Group1and4.label <- c(rep("TP53_WT",times=dim(HiSeq_selected_patients_woTP53mut)[2]),
                      rep("TP53_R175",times=dim(HiSeq_selected_patients_R175)[2]))
...
#Group1and5=====================================================================================
Group1and5 <- transform(merge(HiSeq_selected_patients_woTP53mut, HiSeq_selected_patients_R248, by=0, all=T),
                        row.names = Row.names, Row.names = NULL)
Group1and5.label <- c(rep("TP53_WT",times=dim(HiSeq_selected_patients_woTP53mut)[2]),
                      rep("TP53_R248",times=dim(HiSeq_selected_patients_R248)[2]))
...
#Group1and6================================================
Group1and6 <- transform(merge(HiSeq_selected_patients_woTP53mut, HiSeq_selected_patients_R273, by=0, all=T),
                        row.names = Row.names, Row.names = NULL)
Group1and6.label <- c(rep("TP53_WT",times=dim(HiSeq_selected_patients_woTP53mut)[2]),
                      rep("TP53_R273",times=dim(HiSeq_selected_patients_R273)[2]))
...
#Group1and7==================================================================
Group1and7 <- transform(merge(HiSeq_selected_patients_woTP53mut, HiSeq_selected_patients_R, by=0, all=T),
                        row.names = Row.names, Row.names = NULL)
Group1and7.label <- c(rep("TP53_WT",times=dim(HiSeq_selected_patients_woTP53mut)[2]),
                      rep("TP53_Group3",times=dim(HiSeq_selected_patients_R)[2]))
...
Group1and2.edgeR.filter <- mutate(Group1and2.edgeR.filter,
                                  change = case_when(
...
Group1and7.edgeR.filter <- Group1and7.edgeR
print(length(rownames(Group1and7.edgeR.filter)))
Group1and7.edgeR.filter <- mutate(Group1and7.edgeR.filter,
                                  change = case_when(
                                    (Group1and7.edgeR.filter$P.Value < 0.05)&(Group1and7.edgeR.filter$logFC < -1) ~ 'DOWN',
                                    (Group1and7.edgeR.filter$P.Value < 0.05)&(Group1and7.edgeR.filter$logFC > 1) ~ 'UP',
                                    TRUE ~ 'NOT'))
...
Group1and3.edgeR.filter <- Group1and3.edgeR
print(length(rownames(Group1and3.edgeR.filter)))
Group1and3.edgeR.filter <- mutate(Group1and3.edgeR.filter,
                                  change = case_when(
                                    (Group1and3.edgeR.filter$P.Value < 0.05)&(Group1and3.edgeR.filter$logFC < -1) ~ 'DOWN',
                                    (Group1and3.edgeR.filter$P.Value < 0.05)&(Group1and3.edgeR.filter$logFC > 1) ~ 'UP',
                                    TRUE ~ 'NOT'))
...
Group1and4.edgeR.filter <- Group1and4.edgeR
print(length(rownames(Group1and4.edgeR.filter)))
Group1and4.edgeR.filter <- mutate(Group1and4.edgeR.filter,
                                  change = case_when(
                                    (Group1and4.edgeR.filter$P.Value < 0.05)&(Group1and4.edgeR.filter$logFC < -1) ~ 'DOWN',
                                    (Group1and4.edgeR.filter$P.Value < 0.05)&(Group1and4.edgeR.filter$logFC > 1) ~ 'UP',
                                    TRUE ~ 'NOT'))
...
Group1and4.all.heatmap <- draw_selected_heatmap(choose_matrix = Group1and4.matrix, 
                                                annotation.class = factor(Group1and4.label))
draw(Group1and4.all.heatmap, auto_adjust = FALSE, 
     column_title = "BRCA WT TP53 vs BRCA TP53 R175", column_title_gp = gpar(fontsize = 16))
draw(lgd, x = unit(1, "npc"), y = unit(1, "npc"), just = c("right", "top"))

#Handle data Group1and5===============================================================
Group1and5.edgeR.filter <- Group1and5.edgeR
print(length(rownames(Group1and5.edgeR.filter)))
Group1and5.edgeR.filter <- mutate(Group1and5.edgeR.filter,
                                  change = case_when(
                                    (Group1and5.edgeR.filter$P.Value < 0.05)&(Group1and5.edgeR.filter$logFC < -1) ~ 'DOWN',
                                    (Group1and5.edgeR.filter$P.Value < 0.05)&(Group1and5.edgeR.filter$logFC > 1) ~ 'UP',
                                    TRUE ~ 'NOT'))
...
Group1and5.all.heatmap <- draw_selected_heatmap(choose_matrix = Group1and5.matrix, 
                                                annotation.class = factor(Group1and5.label))
draw(Group1and5.all.heatmap, auto_adjust = FALSE, 
     column_title = "BRCA WT TP53 vs BRCA TP53 R248", column_title_gp = gpar(fontsize = 16))
draw(lgd, x = unit(1, "npc"), y = unit(1, "npc"), just = c("right", "top"))

#Handle data Group1and6===============================================================
Group1and6.edgeR.filter <- Group1and6.edgeR
print(length(rownames(Group1and6.edgeR.filter)))
Group1and6.edgeR.filter <- mutate(Group1and6.edgeR.filter,
                                  change = case_when(
                                    (Group1and6.edgeR.filter$P.Value < 0.05)&(Group1and6.edgeR.filter$logFC < -1) ~ 'DOWN',
                                    (Group1and6.edgeR.filter$P.Value < 0.05)&(Group1and6.edgeR.filter$logFC > 1) ~ 'UP',
                                    TRUE ~ 'NOT'))
...
Group1and6.all.heatmap <- draw_selected_heatmap(choose_matrix = Group1and6.matrix, 
                                                annotation.class = factor(Group1and6.label))
draw(Group1and6.all.heatmap, auto_adjust = FALSE, 
     column_title = "BRCA WT TP53 vs BRCA TP53 R273", column_title_gp = gpar(fontsize = 16))
draw(lgd, x = unit(1, "npc"), y = unit(1, "npc"), just = c("right", "top"))
...
save(Group1and2.edgeR.filter,Group1and7.edgeR.filter,Group1and3.edgeR.filter,
     ...
     file = paste0('./data/','step2_selected','.Rdata'))
